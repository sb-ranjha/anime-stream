...
function SeasonTrending() {
  const { animes } = useAnime();
  const seasonTrendingAnimes = animes.filter(anime => anime.seasonTrending);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold">
            <span className="text-[#f47521]">Season</span> Trending
          </h1>
          <div className="text-sm text-gray-500">
            {seasonTrendingAnimes.length} {seasonTrendingAnimes.length === 1 ? 'Anime' : 'Animes'}
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
          {seasonTrendingAnimes.map((anime) => (
            <Link 
              key={anime._id}
              to={`/anime/${anime._id}`}
        ...
        {seasonTrendingAnimes.length === 0 && (
          <div className="text-center py-16">
            <h3 className="text-xl text-gray-400 mb-4">No season trending anime available</h3>
